...
prefix = "#"
name = "~ XD1 Team"
rdaftar = "مرحبًا ، شكرًا لك على التسجيل😁"
rmenu = "مرحبا😊"
botinfo = ""
limitt = 9999999999
memberLimit = 1
...
async function starts() {
	...
	dms.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*[ مرحبا بك في المجموعة ${mdata.subject} ]*\n*═══════════════════*\n • @${num.split('@')[0]} *اتبع التعليمات!!!*\n*═══════════════════*\n*احترام الجميع*\n*═══════════════════*\n*ملاحظة : لمشاهدة الاوامر اكتب :*\n*#help*`
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			const BotWea2 = ["966XXXXXXXXX@s.whatsapp.net"] // هنا ضع رقمك
			...
			mess = {
				...
				success: '*ناجح...*',
				error: {
					stick: ' *آسف حدث خطأ عند التحويل إلى ملصق*',
					Iv: '*آسف الرابط غير صالح!!*'
				},
				only: {
					group: '*اسف .. هذا الامر فقط في المجموعات!*',
					benned: '*عذرا ، تم حظر رقمك ، يرجى الاتصال بالمالك لفك الحظر!*',
					ownerG: '*عذرًا ، هذا الأمر خاص بالمجموعة المالكة!*',
					ownerB: '*عذرا . هذة الميزة خاصة بالمالك* ',
					premium: '*ميزات مقفلة!!!*',
					userB: ` *مرحبا بك* لمشاهدة الاوامر يجب عليك تفعيل نفسك لل تفعيل اكتب :  #تفعيل`,
					admin: '*𝗠𝗮𝗮𝗳 𝗖𝗼𝗺𝗺𝗮𝗻𝗱 𝗜𝗻𝗶 𝗞𝗵𝘂𝘀𝘂𝘀 𝗔𝗱𝗺𝗶𝗻 𝗚𝗿𝗼𝘂𝗽!*',
					Badmin: '*يجب ان يكون معه رتبت مشرف*',
					publikG: `*𝗠𝗮𝗮𝗳 𝗕𝗼𝘁 𝗦𝗲𝗱𝗮𝗻𝗴 𝗣𝗲𝗿𝗯𝗮𝗶𝗸𝗮𝗻!!!*\n*𝗨𝗻𝘁𝘂𝗸 𝗜𝗻𝗳𝗼 𝗟𝗲𝗯𝗶𝗵 𝗝𝗲𝗹𝗮𝘀𝗻𝘆𝗮 𝗞𝗲𝘁𝗶𝗸*\n*${prefix}infobot*`
				}
			}

			const botNumber = dms.user.jid
			const ownerNumber = ["966XXXXXXXXX@s.whatsapp.net"] // هنا رقم الاونر
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case prefix+'تفعيل':
					dms.updatePresence(from, Presence.composing)
					if (isUser) return reply('رقمك مسجل بالفعل في قاعدة البيانات ')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			//if (!isPublic) return reply(mess.only.publikG)
				...
				case prefix+'premiumlist':
					...
					//if (!isPublic) return reply(mess.only.publikG)
					if (!isUser) return reply(mess.only.userB)
					teks = `╭─「 *المستخدمين ${name}* 」\n`
					no = 0
					for (let prem of premium) {
						no += 1
						teks += `[${no.toString()}] @${prem.split('@')[0]}\n`
					}
					teks += `│+ عدد متسخدمين البريميوم : ${premium.length}\n╰──────⎿ *${name}* ⏋────`
				...
				case prefix+'addprem':
					...
					premium = addpremium
					reply(`*تم اعطاء ${premium} رتبة بريميوم*`)
					break
				case prefix+'removeprem':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
					premium.splice(`${rprem}@s.whatsapp.net`, 1)
					reply(`تمت الإزالة بنجاح wa.me/${rprem} من مستخدمي Premium`)
					break
				case prefix+'unban':
					if (!isOwner)return reply(mess.only.ownerB)
					bnnd = body.slice(8)
					ban.splice(`${bnnd}@s.whatsapp.net`, 1)
					reply(`Nomor wa.me/${bnnd} تم إلغاء الحظر!`)
					break
				case prefix+'block':
				 dms.updatePresence(from, Presence.composing) 
					...
					dms.sendMessage(from, `تم استلام الطلب ، وحظره ${body.slice(7)}@c.us`, text)
					break
				case prefix+'unblock':
                    if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
					...
					dms.sendMessage(from, `تم استلام الطلب ، إلغاء الحظر ${body.slice(9)}@c.us`, text)
				    break 
				...
				if (!isOwner) return reply(mess.only.ownerB)
				  ...
				  await reply(`إعادة تعيين حدود النجاح`)
				break
					case prefix+'limit':
				var found = false
                    ...
                    for (let lmt of limidat) {
                        if (lmt.id === sender) {
                            ...
                            await reply(`*حدد إقامتك: ${limitCounts}*`)
                            found = true
                        }
                    }
                    if (found === false) {
				...
				case prefix+'s':
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg.result, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							fs.writeFileSync(ranp, buffer, (err) => {
								if (err) return reply('فشل ، حدث خطأ ، يرجى المحاولة مرة أخرى في وقت لاحق.')
							})
						...
						reply(`ارسل الصورة مع كتابة ${prefix}sticker أو ضع علامة على الصور التي تم إرسالها بالفعل`)
					}
					await limitAdd(sender) 
					break 
					case prefix+'trigger':
            ...
            break  
               ...
               dms.updatePresence(from, Presence.composing) 
              options = {
          text: `صاحب هذه المجموعة هو : wa.me/${from.split("-")[0]}`,
...
//akhir kreator
		    ...
		    case prefix+'map':
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 XD1 Team 〙═══', members_id, true)
					break
				...
				case prefix+'clearall':
					...
					reply(`*_Sukses delete all chat XD1 Team_*`)
					break
                                case prefix+'bcgc':
					dms.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							sendMess(_.jid, `*「 مجموعة البث 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
						}
						reply('تم الارسال ... للمجموعات')
					}
					break
				case prefix+'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *XD1 Team رسالة جماعية* ]\n\n${body.slice(4)}`)
						}
						reply('تم الارسال ...')
					}
					break
				...
				//if (!isPublic) return reply(mess.only.publikG)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('تريد اضافة من??')
					if (args[0].startsWith('08')) return reply('استخدم رمز البلد الخاص')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						dms.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('لا يمكن اضافة العضو يبدو انه خاص')
					}
					break
			    case prefix+'kick':
			    if (isBanned) return reply(mess.only.benned)    
				...
				case prefix+'toimg':
				if (isLimit(sender)) return reply(limitend(pushname2))
					if (!isQuotedSticker) return reply(' الرد على الملصق')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						dms.sendMessage(from, buffer, image, {quoted: mek, caption: 'تم التحويل بنجاح'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
				...
				if (!isGroup) return reply(mess.only.group)
				if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('هممم')
					if ((args[0]) === 'on') {
				...
				//if (!isPublic) return reply(mess.only.publikG)
					...
					if (args.length < 1) return reply('هممم')
					if ((args[0]) === 'on') {
						if (isAnime) return reply('وضع الرسوم المتحركة نشط')
						anime.push(from)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`\`\`\`✓تم تفعيل وضع animé في المجموعة بنجاح\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`\`\`\`✓نجح في تعطيل وضع animé في المجموعة\`\`\` *${groupMetadata.subject}*`)
					} else {
						reply('On untuk mengaktifkan, Off untuk menonaktifkan')
					}
					break
				...
				if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('للتفعيل اكتب : #antilink on')
					if ((args[0]) === 'on') {
						if (isAntiLink) return reply('مانع الروابط قيد التشغيل ..')
						antilink.push(from)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(antilink))
						reply(`\`\`\`✓Sukses mengaktifkan fitur anti link di group\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						if (!isAntiLink) return reply('مانع الروابط مغلق!')
...
//setting bot
				...
				case prefix+'setpref':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`تم تغيير البادئة بنجاح إلى : ${prefix}`)
					break 
				...
				case prefix+'setnamebot':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					name = body.slice(12)
					reply(`تم تغيير اسم الروبوت بنجاح إلى : ${name}`)
					break 
				case prefix+'setreply':
					...
					if (args.length < 1) return
					rmenu = body.slice(10)
					reply(`تم تغيير الرد بنجاح إلى : ${rmenu}`)
				break 
////////////
				case prefix+'wait':
				if (isBanned) return reply(mess.only.benned)    
		...
		if (isLimit(sender)) return reply(limitend(pushname2))
		 ...
		 case prefix+'tomp3':
				...
				case prefix+'setppbot':
				    ...
				    dms.updatePresence(from, Presence.composing) 
					if (!isQuotedImage) return reply(`أرسل صورة مع شرح ${prefix}setbotpp أو ضع علامة على الصور التي تم إرسالها بالفعل`)
					enmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await dms.downloadAndSaveMediaMessage(enmedia)
					await dms.updateProfilePicture(botNumber, media)
					reply('تم انشاء الملف الشخصي الجديد🙂')
					break
